...
function startNotifyServer(api) {
    const http = require("node:http");
    const server = http.createServer((req, res) => {
        // POST /api/zoo-notify — 旧 QQ Bot 通知（保留兼容）
        if (req.method === "POST" && req.url === "/api/zoo-notify") {
            ...
            req.on("end", () => {
                try {
                    ...
                    api.logger.info(`${LOG_PREFIX} 🔔 转发通知到 ${agent}: ${pid}`);
                    const QQ_OPENID = {
                    ...
                    try {
                        ...
                        api.logger.warn(`${LOG_PREFIX} 通知 ${agent} 发送失败: ${e.stderr?.toString().slice(0, 100) || e.message}`);
                    }
        ...
        // ── POST /api/sessions-send: daemon 专用，发送到 agent main session ──
            ...
            req.on("end", async () => {
                try {
                    ...
                    // 从 zoo_members.yaml 读取 session key
                    ...
                    if (existsSync(yamlPath)) {
                        try {
                            ...
                            sessionKey = cfg?.members?.[agent]?.session?.key || sessionKey;
                        } catch (e) {
                            // fallback to agent:xxx:main
                        }
                    }
    ...
    server.listen(NOTIFY_PORT, "127.0.0.1", () => {
        api.logger.info(`${LOG_PREFIX} 🎯 ZooNotify HTTP 已启动: http://127.0.0.1:${NOTIFY_PORT}`);
    });
}
// ── Dashboard 进程管理 ─────────────────────────────────────────────────
const DASHBOARD_PATH = process.env.ZOO_DASHBOARD_SCRIPT ||
    "/Users/zoo/workspace/code/feida_zoo/dashboard/app_enhanced.py";
const DASHBOARD_PYTHON = process.env.ZOO_DASHBOARD_PYTHON ||
...
function startDashboard(api) {
    if (dashboardCrashCount >= DASHBOARD_MAX_CRASH) {
        api.logger.error(`${LOG_PREFIX} 🚨 Dashboard 连续崩溃 ${dashboardCrashCount} 次，停止自动重启`);
        return;
    }
    api.logger.info(`${LOG_PREFIX} 启动 Dashboard: ${DASHBOARD_PATH} (端口 ${DASHBOARD_PORT})`);
    ...
    dashboardProcess.on("exit", (code) => {
        if (code !== 0 && code !== null) {
            dashboardCrashCount++;
            api.logger.warn(`${LOG_PREFIX} ⚠️ Dashboard 异常退出 (code=${code})，${CRASH_RESTART_DELAY_MS}ms 后重启`);
    ...
    dashboardProcess.on("error", (err) => {
        dashboardCrashCount++;
        api.logger.error(`${LOG_PREFIX} 🚨 Dashboard 启动失败: ${err.message}`);
        if (dashboardCrashCount < DASHBOARD_MAX_CRASH) {
            setTimeout(() => startDashboard(api), CRASH_RESTART_DELAY_MS);
        }
    });
...
export function registerGatewayStart(api) {
...
function startDaemon(api, config) {
    if (crashCount >= MAX_CRASH_COUNT) {
        api.logger.error(`${LOG_PREFIX} 🚨 ZooMesh 连续崩溃 ${crashCount} 次，停止自动重启，请人工排查！`);
        return;
    }
    const env = {
        ...process.env,
    ...
    api.logger.info(`${LOG_PREFIX} 启动 ZooMesh 守护进程: ${config.daemonPath} (端口 ${config.httpPort})`);
    ...
    daemonProcess.on("exit", (code) => {
        if (code !== 0 && code !== null) {
            crashCount++;
            api.logger.warn(`${LOG_PREFIX} ⚠️ ZooMesh 异常退出 (code=${code})，崩溃次数=${crashCount}/${MAX_CRASH_COUNT}，${CRASH_RESTART_DELAY_MS}ms 后重启`);
    ...
    daemonProcess.on("error", (err) => {
        crashCount++;
        api.logger.error(`${LOG_PREFIX} 🚨 ZooMesh 启动失败: ${err.message}，崩溃次数=${crashCount}/${MAX_CRASH_COUNT}`);
        if (crashCount < MAX_CRASH_COUNT) {
            setTimeout(() => startDaemon(api, config), CRASH_RESTART_DELAY_MS);
        }
    });
...
async function waitForHealth(url, timeoutMs, api) {
    ...
    api.logger.warn(`${LOG_PREFIX} ⚠️ ZooMesh 健康检查超时 (${timeoutMs}ms): ${url}`);
}
//# sourceMappingURL=gateway-start.js.map